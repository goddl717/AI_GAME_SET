...
class Game2_easy : AppCompatActivity() {
    var cnt = 0
    var num = -1
    var timer : Timer? = null
    var arr_random = arrayOfNulls<Int>(24)
        ...
        timer = kotlin.concurrent.timer(period = 1000) {
            ...
            if (i==45)
            {
                runOnUiThread {
                    ...
                    btn_game2_right.isEnabled = false
                    this.cancel()
                    Toast.makeText(this@Game2_easy, "종료 되었습니다.", Toast.LENGTH_SHORT)



                    //Thread.sleep(300)
                    //println("total " + cnt)
                    //val intent = Intent(this@Game2_easy, Game2_result::class.java)
                    //intent.putExtra("score", cnt.toString())
                    //intent.putExtra("type", "2-back")
    ...
    private fun btn_left_click() {
        if (num ==0 || num ==1){
            makeText1(this,"3번째 부터 누를수 있습니다.",Toast.LENGTH_SHORT).show()
        }
        else if (arr_random[num] == arr_random[num-2]) {
            println("current :")
            println(num )
